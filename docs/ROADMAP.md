...
| Name the remaining opaque leaves against a vocabulary | **PR #55 open** | review. 10 → 4; the four left are the dust's, and they need a representation change rather than another op — see the row on a scalar channel | GPU rendering |
...
| A scalar channel in `VecField`, and `Hash`/`Gather` | **next** | nothing. The dust's four leaves all start from a hash of the instance origin and a gather from the BOOM table, and a vector field has no scalar-valued nodes to hold either. Until it lands no outside language can describe the dust | Declarative scene DSL |
