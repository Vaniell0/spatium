...
- **[course]** Structural fields — one `Field` type that is an expression tree, in which an opaque callable is a *leaf* rather than an alternative to the tree. The distinction is not pedantry: under "tree or callable" a user picks a representation at the declaration site and silently forfeits caching, lowering and hoisting, whereas under "tree with opaque leaves" composing a structural field with an opaque one keeps the outer structure and loses only the one leaf. "Is this lowerable" is then derived by walking for opaque leaves, not declared and not kept in sync by hand.
...
- **[want]** Exercise `mesh/conform.hpp`'s `conform_to_surface()` and `mesh/scatter.hpp`'s `scatter_on_surface()` (mesh-based geodesic-Voronoi placement, built the same day as an alternative to the analytic path) in a real demo — they compile and pass their own tests but nothing in the tree actually calls them yet.
