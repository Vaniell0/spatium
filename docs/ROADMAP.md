...
- ~~**[course]** Instancing in the trace~~ and ~~**[course]** Structural fields~~ — **the library half landed 2026-09-16**, merged as #42 (fields), #45 (`cook()`), #44 (the instance leaf). What that means concretely, since "instancing" turned out to name three separable things:
  ...
  **Still open, and it is the part that moves a number:** no renderer consumes any of it. The dust is 56% of the frame (2.02 s with, 0.895 s without) and still tessellates. Wiring `BVH<Instanced<Box>>` into `donut_demo` is connection work, not design — everything under it is measured. Raising the particle count waits for that, since the frame is linear in count today (19 800 → 1.94 s, 59 400 → 4.27 s).
