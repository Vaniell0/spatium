...
## Catalogue: kinds of question, and what each one needs

Written 2026-09-18. The point of the catalogue is that "RSC" is not one
capability. It is several kinds of question that happen to share a
registry, and they want different model sizes, different architectures and
different corpora. Conflating them is how the wrong tool gets pointed at a
problem — measured examples of exactly that are below.

Marked throughout: **measured**, **designed**, or **not attempted**.

### 1. Dispatch — "which implementation for this input"

**Status: measured, seven domains.** The only kind that is actually built
and trained. Input is a feature vector, output is one op from the registry.

*Model:* one hidden layer, 64 units, 22 features, 10 ops in the base. Small
because the task is nearly a decision tree: each input determines its label
and nothing is carried across steps.

*Corpus:* generated per domain, with ground truth from comparing the
candidate implementations against each other rather than from a hand label
— `precision_task.hpp` compares `solve_cubic_f64` against
`solve_cubic_real50` and takes the disagreement as the signal.

*Deployment:* a distilled decision tree per domain, not the network.
Measured 2026-09-18 — the network costs 260x an `if` and cannot be made
cheaper by shrinking (hidden 8 is still 51x), while one tree per domain at
depth 8 costs 3.7x and gives up 0.005 accuracy. A single tree over all four
domains is both worse and dearer, because the domain is known at the call
site and should not cost two levels of tree to re-derive.
(`rsc/tools/dispatch_cost.cpp`, `rsc/tools/distill_tree.cpp`.)

### 2. Calibration — "what parameter"

**Status: designed and partly built. Not a model, deliberately.** Classical
optimization against the same gate. Recorded here because it is the half
that does *not* transfer between spaces: a step size fitted on a unit
sphere will not hold in hyperbolic space, while the dispatch above is
space-agnostic by construction.

### 3. Surrogates — "which closed-form family, and with what coefficients"

**Status: not attempted; no approximation families are registered yet.**
Once Padé, Chebyshev, Prony and friends are ordinary registry entries, this
collapses into kind 1 plus kind 2 with no new architecture: dispatch picks
the family, calibration fits it, the gate compares against exact Spatium
ground truth and declines when nothing passes.

*Model:* tiny again — a handful of families to choose between.

### 4. Synthesis — "find a sequence of operations meeting a specification"

**Status: not attempted. This is the AlphaDev-shaped one.** The registry is
the instruction set and a chain is the program, so what corresponds to
AlphaTensor's decompositions here is a *sequence* of registered operations
rather than a single choice.

*The oracle is free, which is the unusual part.* A reference implementation
already exists for anything worth searching for, so a candidate chain is
graded by running it against the reference on random inputs. No labels, and
nothing to game.

*Constraints are the interesting input, and they are not hints.* "Solve
this using only addition" is a *restricted action space*, not a nudge —
search over `{add}` alone. So is "without branching". A refusal is a
result: "unreachable in N steps under this restriction" is an answer.

*Model:* different architecture from kind 1. Dispatch classifies a state;
synthesis needs to score a *partial program against a goal*, which is a
value function — input is (partial chain, target), output is a scalar
estimate of distance remaining. That is the piece ROADMAP calls missing,
and it is missing in a strong sense rather than a convenient one: measured
2026-09-18, substituting the policy's action log-probabilities for it makes
search *worse than no heuristic at all*, expanding 1.6-1.7x more nodes than
uninformed breadth-first while solving less often
(`rsc/tools/search_heuristic.cpp`).

*Build order, from the same measurement:* start with no model. Under a
tight constraint the action space is small, and on small spaces breadth-
first beats a learned heuristic on every axis. Add a model only where a
measurement shows search running out of budget — which is where the
constraint is loose and the space is large.

*Corpus:* specifications with known solutions, generated by walking the
registry backwards from a result.

*The caveat that keeps the ambition honest:* this composes registered
operations and cannot invent one. "A new algorithm" means a sequence nobody
wrote down, in the sense AlphaDev found shorter sorting sequences out of
existing instructions.

### 5. Rewriting an IR — "the same result, cheaper"

**Status: not attempted; the oracle is already queued for another reason.**
The scene trace is a flat op graph, and a rewrite must produce a bit-exact
identical result — which the POD interpreter planned for the ABI work
already checks. One artifact, two uses.

*How it differs from kind 4, and it matters:* bit-exact equality admits no
improvement on correctness, so the objective is cost subject to identity.
Synthesis can invent; rewriting can only economise.

*Model:* different again — the input is a *graph*, not a vector, so this
wants per-node scoring or a small graph network rather than an MLP over
fixed slots.

*Corpus:* graphs from real scenes plus randomly generated rewrites,
labelled by measured evaluation cost.

### 6. Building a structure — "where to split"

**Status: not attempted; needs a bound concept first.** A BVH and a ball
tree differ only in the bound, so one hierarchy over a bound concept —
bound a set, merge two bounds, test a query — serves axis-aligned boxes,
geodesic balls, OBBs and k-DOPs alike.

*The oracle is free and, unlike kind 5, allows beating the reference:*
build the tree, run queries, time them. There is no output to match, only
a cost to lower.

*Why this is the best target for a learned heuristic of the three
searching kinds:* all three conditions measured on 2026-09-18 line up here
and only here. The state does not collapse (different split sequences give
genuinely different trees, unlike numeric chains where 9 765 625 paths at
depth 10 reduce to 13 836 states). Expanding a node is expensive — running
a mesh operation, milliseconds — so a 600 ns policy evaluation is free
against it, where over numbers it cost more than ten nodes. And the reward
is a measurement rather than a judgement.

*And it is where the transfer claim gets tested cheaply:* learn a split
policy over the bound concept on Euclidean boxes, then run it against
geodesic balls. Whether it still helps is a measurement.

### What is shared, and what is not

Shared: the registry, `grade()`, the gate against exact ground truth, and
the base-plus-custom deployment split.

Not shared, and this is the part worth keeping straight: **the model.**
Kind 1 wants a tiny classifier and ships as a tree. Kind 4 wants a value
function over partial programs. Kind 5 wants something that reads a graph.
Kind 6 wants a scorer over a set of primitives. Different volumes,
different architectures, different corpora — one registry underneath.
