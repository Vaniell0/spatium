...
- `include/spatium/physics/mechanics/` — units (compile-time SI), PointMass/RigidBody, forces, integrators (Euler/RK4/Lie-group/LGVI), symplectic manifolds, variational integrators, geometric continuum mechanics, IPC contact barrier + XPBD, certified continuous collision (narrow_phase.hpp `distance_bound` — a lower bound on distance, exact for sphere/torus, branch and bound for a `LipschitzChart`, `|f|/L` for a `LipschitzImplicit`; rigid_contact.hpp `sweep_sphere_surface` advances by it, a miss only when proved; `first_contact` searches a chart's `ChartCellTree` in parameters x time, each cell a ball and, when the chart bounds its second derivatives, a slab between two planes -- a floor of second order that a graze barely erodes; surface_ccd.hpp `first_contact` for two surfaces moving linearly -- pairs of cells x time, each pair's interval cut exactly by the ball's quadratic and the slabs' lines, time split only once the cells are as small as asked; compared against TDIB-CCD in `benchmarks/external/tdib_compare.cpp`), wave PDE time-stepping (wave_string.hpp — 1D clamped string, wave_membrane.hpp — 2D clamped rectangular membrane; both explicit central-difference/"leapfrog" steppers with closed-form fundamental-frequency helpers, purpose-built rather than routed through `algebra/ode.hpp` — see either header's own comment for why). Doesn't yet use `Result<T>` for fallible ops (see `docs/architecture.md`'s "Header-only spine, and three principled exceptions")
...
- `examples/` — geometry_demo, showcase, sets_demo, spd_relationship_demo, scene_demo (text/SVG); donut_demo (spatium::io::build's declarative DSL getting-started demo — console by default, `--photo` for PNG; see `docs/getting-started-dsl.md`; the scene itself is built in `donut_scene.hpp`, so a test can build the same one); atom_demo + primitives_demo (Vulkan; primitives_demo dispatches `--scene primitives|torus|klein`); blackhole_live (black holes described with `SpacetimeScene` and traced on the device; `--live` opens a window rendering at 144p by default, the whole frame each time, scaled to the window, with a few settings -- one hole or a pair, spin, the pair's masses, separation and inspiral, the disk, camera angles, exposure, time, resolution up to the window's -- and a button that saves the view re-rendered at 1920x1080; `--check` sends rays through the metric on the device, on the host in float and in double and reports the end points' spread and the device time; `--bench` prints a frame's cost by part (matter move/pack, tree boxes/sort/nodes, trace) for one hole and a pair, 0-1M particles, 144p and 540p; `--dust-check` moves dust particles on the device and compares them with the host in double; `--shadow-check` measures a Schwarzschild shadow's angular radius against 3 sqrt(3) M / D sqrt(1 - 2M/D); `--frame PATH` renders one frame; `--video DIR` writes a frame sequence for ffmpeg, on any Vulkan device, a rented NVIDIA card included), donut_live (the donut traced by a Vulkan compute shader: headless by default, checked against the host's fp32 trace and the fp64 trees; `--live` opens a window with a free-flying camera and ImGui; `--dust-check [--dust N]` moves the scene's Scatters on the device and compares every slot with `cook()`, then builds the instance tree on the host and on the device and checks the device tree against brute force; in `--live` every structural Scatter is moved by its kernel each frame and the tree rebuilt on the device, with play/rate in ImGui); wormhole_demo, tumbling_body_demo, parametric_analytical_demo, geodesic_curvature_grid_demo, geodesic_procgen_demo, hyperbolic_tessellation_demo, wave_ca_demo, collatz_demo, burning_ship_demo, native_collision_demo, ball_pit_demo, napkins (napkins dropped or thrown onto a sphere, a floor and each other -- `napkin_scene.hpp`: cloth-cloth contact as constraints in the XPBD iterations, surface_ccd's vertex-face and edge-edge queries behind a coplanarity filter as the guarantee, and an independent count of edges piercing triangles; `--throw`, `--rate`, `--trace`), cloth_live (an XPBD sheet on a sphere, torus or Klein bottle -- the removed cloth_visual_demo's scene, kept apart in `cloth_scene.hpp`; `--measure` counts tunnelling moves by the exact crossings, depth before and after contact, stretch and explosions for analytic projection and a tessellated obstacle), sound_synthesis_demo, terminal_donut_demo (ASCII, `render::terminal_canvas.hpp`, not Vulkan), cloth_sphere_probe (offline/Vulkan raytracers and diagrams — see `spatium_add_example()` in `cmake/SpatiumTarget.cmake`)
